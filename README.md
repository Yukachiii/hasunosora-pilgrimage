...
- ローカル管理画面: `http://127.0.0.1:8766/admin/`

## 管理画面を起動する

普段はクライアントPCでプロジェクト直下の `start-admin.bat` をダブルクリックしてください。`yuimarine@192.168.0.4`へSSH接続し、サーバー側の8766番をクライアントPCの`127.0.0.1:8766`へ転送します。蓮ノ旅の管理画面だと確認できた場合だけブラウザを開くため、別のアプリを誤って表示しません。パスワードを求められた場合はSSHのパスワードを入力してください。停止するときは、起動した画面で `Ctrl+C` を押すかウィンドウを閉じます。
...
管理画面はSSHトンネル経由で`127.0.0.1`にだけ接続し、8766番をインターネットへ公開しません。
...
受付プロセスは必ず `127.0.0.1:8790` だけで待ち受けます。インターネットへ公開するときは、Tailscale FunnelやCloudflare TunnelなどのHTTPSリバースプロキシからこのポートだけへ接続してください。管理画面の `8766` 番ポートはTunnelへ設定しないでください。
...
写真投稿では、選択した写真にGPS情報があればブラウザ内で最寄りの登録スポットを候補にします。読み取った緯度・経度はフォーム項目として送信しません。受付時は元画像を保存せず、向きを補正して最大2560pxのWebPへ再生成します。再生成した公開候補にはEXIF、端末名、元ファイル名を残しません。生のIPアドレスも保存せず、1日5件の制限に使う日替わりハッシュだけを残します。受付サーバーからの外部API呼び出しは投稿1回につきTurnstile確認1回です。一時的な通信失敗やHTTP 5xxなどの場合だけ、同じ冪等キーで1回再試行するため最大2回になります（ローカル開発時の確認省略を除く）。これとは別に、投稿画面のTurnstileウィジェットがCloudflareと通信します。

Turnstileの最終確認に失敗した場合は、原因を次の非公開ログで確認できます。

```powershell
Get-Content .\private\community-submissions\diagnostics\turnstile-failures.jsonl -Tail 10
```

ログへ保存するのは発生時刻、失敗段階、HTTPステータス、許可済みのCloudflareエラーコードなどの診断情報だけです。秘密鍵、確認トークン、IPアドレス、Origin、投稿内容、レスポンス本文、例外メッセージは保存しません。ログは512KiBごとにローテーションし、最大3世代を保持します。
...
ローカル管理画面の「API使用状況」では、投稿受付サーバーについて次を確認できます。

- 投稿受付サーバーの稼働状態
- 今日・今月・計測開始以降の投稿試行、受付、失敗
- Turnstile Siteverifyの実リクエスト、成功、失敗、再試行
- 現在保持している審査待ち、取込済み、見送りの件数

集計は`COMMUNITY_SUBMISSIONS_DIRECTORY`内の非公開ファイル
`diagnostics/api-usage.json`へ保存します。計測を導入した時点からの数値で、過去分は
さかのぼって加算しません。IPアドレス、Origin、確認トークン、投稿内容、画像情報は
記録しません。

公開ページの地図・経路検索はMapboxをブラウザから直接利用しているため、ローカル
サーバーでは請求対象の正確な回数を取得できません。管理画面のリンクからMapbox公式
