...
test("public deployment supplies the Mapbox token expected by the public app", async () => {
  const [entry, publicApp, workflow, envExample] = await Promise.all([
  ...
  assert.ok(entry.includes("UiTrialApp"));
  assert.ok(publicApp.includes(tokenName));
